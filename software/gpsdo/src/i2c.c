...
#else   // I2C2_MODE_MASTER
static i2c_slave_addr_isr_t pfI2C2SlaveAddrISR = NULL;
static i2c_slave_tx_data_isr_t pfI2C2SlaveTXDataISR = NULL;
static i2c_slave_rx_data_isr_t pfI2C2SlaveRXDataISR = NULL;

void _i2c2_isr()
{
    uint32_t ulFlags = I2C2->IFC;

    if(ulFlags & I2C_IFC_BUSHOLD)
    {
        switch(I2C2->STATE & _I2C_STATE_STATE_MASK)
        {
            case I2C_STATE_STATE_ADDR:
            {
                I2C2->CMD = I2C_CMD_CLEARTX;

                volatile uint8_t ubAddress = I2C2->RXDATA;

                if(pfI2C2SlaveAddrISR && pfI2C2SlaveAddrISR(ubAddress))
                    I2C2->CMD = I2C_CMD_ACK;
                else
                    I2C2->CMD = I2C_CMD_NACK;
            }
            ...
            case I2C_STATE_STATE_ADDRACK:
            {
                I2C2->TXDATA = pfI2C2SlaveTXDataISR ? pfI2C2SlaveTXDataISR() : 0xFF;
            }
            break;
            case I2C_STATE_STATE_DATA:
            {
                volatile uint8_t ubData = I2C2->RXDATA;

                if(pfI2C2SlaveRXDataISR && pfI2C2SlaveRXDataISR(ubData))
                    I2C2->CMD = I2C_CMD_ACK;
                else
                    I2C2->CMD = I2C_CMD_NACK;
            }
...
void i2c2_init(uint8_t ubAddress, uint8_t ubSCLLocation, uint8_t ubSDALocation)
{
    if(ubSCLLocation > AFCHANLOC_MAX)
        return;
    ...
    I2C2->CTRL = I2C_CTRL_TXBIL_EMPTY | I2C_CTRL_SLAVE;
    I2C2->CLKDIV = 1;
    I2C2->SADDR = (ubAddress << _I2C_SADDR_ADDR_SHIFT);
    I2C2->SADDRMASK = (0x7F << _I2C_SADDRMASK_MASK_SHIFT);
    I2C2->ROUTEPEN = I2C_ROUTEPEN_SCLPEN | I2C_ROUTEPEN_SDAPEN;
    I2C2->ROUTELOC0 = ((uint32_t)ubSCLLocation << _I2C_ROUTELOC0_SCLLOC_SHIFT) | ((uint32_t)ubSDALocation << _I2C_ROUTELOC0_SDALOC_SHIFT);

    I2C2->IFC = _I2C_IFC_MASK; // Clear all flags
    IRQ_CLEAR(I2C2_IRQn); // Clear pending vector
    IRQ_SET_PRIO(I2C2_IRQn, 2, 0); // Set priority 2,0
    IRQ_ENABLE(I2C2_IRQn); // Enable vector
    I2C2->IEN |= I2C_IEN_BUSHOLD; // Enable BUSHOLD flag
...
void i2c2_set_slave_addr_isr(i2c_slave_addr_isr_t pfISR)
{
    pfI2C2SlaveAddrISR = pfISR;
}
void i2c2_set_slave_tx_data_isr(i2c_slave_tx_data_isr_t pfISR)
{
    pfI2C2SlaveTXDataISR = pfISR;
}
void i2c2_set_slave_rx_data_isr(i2c_slave_rx_data_isr_t pfISR)
{
    pfI2C2SlaveRXDataISR = pfISR;
}
#endif  // I2C2_MODE_MASTER