...
static uint8_t si5351_read_register(uint8_t ubRegister)
{
    ...
    {
        i2c1_write_byte(SI5351_I2C_ADDR, ubRegister, I2C_RESTART);
        return i2c1_read_byte(SI5351_I2C_ADDR, I2C_STOP);
    }
}
static void si5351_write_register(uint8_t ubRegister, uint8_t ubValue)
{
    ...
    ATOMIC_BLOCK(ATOMIC_RESTORESTATE)
    {
        i2c1_write(SI5351_I2C_ADDR, pubBuffer, 2, I2C_STOP);
    }
}
...
uint8_t si5351_init()
{
    if(!i2c1_write(SI5351_I2C_ADDR, NULL, 0, I2C_STOP)) // Check ACK from the expected address
        return 0;
    ...
    REF_SYNTH_OUT_DIS(); // Disable all outputs by hardware
